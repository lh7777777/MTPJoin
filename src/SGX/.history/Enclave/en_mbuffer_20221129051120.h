...
public:
    ...
    void WriteBuffer(int buffer_id,char mbpool[1024][4096]) {
...
public:
    ...
    int FixPage(int node_id, EBuf_pool &thebuffer,MBuf_des* mbdes,char mbpool[1024][4096]) {
        MBuf_id mbuf_id;
        ...
        if (page2buffer.find(page_id) != page2buffer.end()) { // 在缓冲区
            ...
            memcpy(thebuffer.field, mbpool[mbid]+offset*1024, 1024);
        ...
        memcpy(thebuffer.field, mbpool[target_bid]+offset*1024, 1024);
    ...
    // 更新某个page
    void UpdatePage(int node_id, EBuf_pool &thebuffer,MBuf_des* mbdes,char mbpool[1024][4096]) {
        ...
        if (page2buffer.find(page_id) != page2buffer.end()) { // 在缓冲区
            ...
            IncHitNum();
            int mbid = page2buffer[page_id];
            memcpy(mbpool[mbid]+offset*1024, thebuffer.field, 1024);
            ...
            memcpy(mbpool[target_bid]+offset*1024, thebuffer.field, 1024);
    ...
    // 将所有的 dirty缓冲块写回磁盘
    void WriteDirtys(MBuf_des* mbdes,char mbpool[1024][4096]) {
