...
public:
    ...
    void WriteBuffer(int buffer_id,MBuf_des* mbdes,MBuf_pool* mbpool) {
        //LOG_DEBUG("EBuffer.WriteBuffer, buffer_id", buffer_id);
        printf("EBuffer.WriteBuffer\n");
        mbuffer->UpdatePage(buffer2node[buffer_id], ebuf_pool[buffer_id],mb);      
    }
    // 从mbuffer读一个page到ebuffer
    void ReadPage(int node_id, EBuf_pool &thebuffer,MBuf_des* mbdes,MBuf_pool* mbpool) {
        //LOG_DEBUG("EBuffer.ReadPage, node_id", node_id);
        /*if (mbuffer->FixPage(node_id, thebuffer) == -1)
            FAIL;*/
        mbuffer->FixPage(node_id, thebuffer,mb);
    }
    ...
    void WriteDirtys(MB* mb) {
        ...
        for (int bid = 0; bid < size; bid++) {
            if (ebuf_des[bid].dirty) {
                //int ret =
                mbuffer->UpdatePage(buffer2node[bid], ebuf_pool[bid],mb);
                //if (ret == -1)
                    //FAIL;
                ebuf_des[bid].dirty = false;
            }
        }
        mbuffer->WriteDirtys(mb);
    }
};
