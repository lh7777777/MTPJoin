...
namespace BAT {
	...
	public:
		...
		virtual E* find(K const &,MBuf_des* mbdes,MBuf_pool* mbpool);//查找内容
		virtual bool insert(E const&,MBuf_des* mbdes,MBuf_pool* mbpool);//插入
	...
	protected:
		...
		void solveOverFlow(BAddTreeLeafNode<K, E>*,MBuf_des* mbdes,MBuf_pool* mbpool);//解决插入上溢现象 参数为节点
	...
	template<typename K, typename E>
	inline E* BAddTree<K, E>::find(K const &k,MBuf_des* mbdes,MBuf_pool* mbpool)
	{
		...
		while (!v->isLeaf())
		{
			...
			if (ebuffer->node2buffer.find(node_id) != ebuffer->node2buffer.end()) { // 在缓冲区
				...
				ebuffer->ReadPage(node_id,tmp_buffer,mbdes,mbpool);
	...
	template<typename K, typename E>
	inline bool BAddTree<K, E>::insert(E const &e,MBuf_des* mbdes,MBuf_pool* mbpool)
	{
	...
	template<typename K, typename E>
	inline void BAddTree<K, E>::solveOverFlow(BAddTreeLeafNode<K, E>*v,MBuf_des* mbdes,MBuf_pool* mbpool)
	{
