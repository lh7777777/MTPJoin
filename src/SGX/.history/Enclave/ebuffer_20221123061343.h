...
public:
    ...
    void WriteBuffer(int buffer_id) {
        ...
        UpdatePage(buffer2node[buffer_id], ebuf_pool[buffer_id]);
    }
    ...
    void ReadPage(int node_id, EBuf_pool &thebuffer) {
        ...
        /*if (mbuffer->FixPage(node_id, thebuffer) == -1)
            FAIL;*/
        //mbuffer->FixPage(node_id, thebuffer);
        FixPage(node_id, thebuffer);
    }
    ...
    void WriteDirtys() {
        ...
        for (int bid = 0; bid < size; bid++) {
            if (ebuf_des[bid].dirty) {
                ...
                UpdatePage(buffer2node[bid], ebuf_pool[bid]);
                //if (ret == -1)
                    //FAIL;
