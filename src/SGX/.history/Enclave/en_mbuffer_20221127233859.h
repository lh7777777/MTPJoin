...
public:
    ...
    void WriteBuffer(int buffer_id) {
        ...
        ocall_writepage(buffer2page[buffer_id], mbuf_pool[buffer_id].field);
    }
    // 从磁盘读一个page到mbuffer
...
public:
    MBuffer(int okk/*DataStorageMgr *ds_mgr*/) {
        ...
        hit_num = tot_page_update_num = okk;
    }
    ~MBuffer() {
        //LOG_DEBUG("~MBuffer");
        delete[] mbuf_pool;
    ...
    int FixPage(int node_id, EBuf_pool &thebuffer) {
        ...
        if (page2buffer.find(page_id) != page2buffer.end()) { // 在缓冲区
            ...
            memcpy(dstStringTemp, (mbuf_pool[mbid].field)+offset*1024, 1024);
        ...
        ReadPage(page_id, mbuf_pool[target_bid]);
        ...
        memcpy(dstStringTemp, (mbuf_pool[target_bid].field)+offset*1024, 1024);
        if(!aes_decrypt(dstStringTemp,key,thebuffer.field,1024))
        {
            printf("decrypt error\n");
        }
    ...
    void UpdatePage(int node_id, EBuf_pool &thebuffer) {
        ...
        if (page2buffer.find(page_id) != page2buffer.end()) { // 在缓冲区
            ...
            memcpy((mbuf_pool[mbid].field)+offset*1024, dstStringTemp, 1024);
            ...
            if (size >= MBUFFER_NUM_MAX_SIZE) { // 缓冲区满
                ...
                if (mbuf_des[target_bid].dirty) { // dirty
                    printf("踢入disk\n");
                    ocall_writepage(buffer2page[target_bid], mbuf_pool[target_bid].field);
            ...
            memcpy((mbuf_pool[target_bid].field)+offset*1024, dstStringTemp, 1024);
            mbuf_des[target_bid].update(page_id, target_bid, 0, false);
            SetDirty(target_bid);
            do_insert ? LRUInsert(target_bid) : LRUUpdate(target_bid);
            do_insert ? size += 1 : 0;
    ...
    void WriteDirtys() {
        ...
        for (int bid = 0; bid < size; bid++) {
            if (mbuf_des[bid].dirty) {
                ocall_writepage(buffer2page[bid], mbuf_pool[bid].field);
