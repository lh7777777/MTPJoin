		...
		BAddTreeLeafNode<K, E>*v = static_cast<BAddTreeLeafNode<K, E>*>(this->m_root);//c++式静态转换
			...
			while (!v->isLeaf())
			{
				...
				if (ebuffer->node2buffer.find(node_id) != ebuffer->node2buffer.end()) { // 在缓冲区
					...
					node = (BAddTreeLeafNode<K, E>*) ebuffer->ebuf_pool[bfid].field;
					cout<<"找到的非叶子nodeid,bufferid: "<< node->node_id <<","<<bfid<<endl;
	...
	inline void BAddTree<K, E>::solveOverFlow(BAddTreeLeafNode<K, E>*v)
	{
		while (v->child.size()>m_order)//上溢
		{
			...
			if (v->isLeaf()) {//情况一 叶节点复制上溢点部分并更新链表
				...
				cout<<"叶子裂完了！"<<endl;
				//cout<<"_r->key.size:"<<_r->key.size()<<endl;
/*
				// 更新旧的叶子节点ebuffer
				int old_id = v->node_id;
				int old_bid = ebuffer->node2buffer[old_id];
				cout<<"旧的叶子nodeid: "<<old_id<<endl;
				memcpy(ebuffer->ebuf_pool[old_bid].field,v,EBUFFER_SIZE);
				//ebuffer->ebuf_des[target_bid].update(node_id, target_bid, 0, false);
				ebuffer->SetDirty(old_bid);
				ebuffer->LRUUpdate(old_bid);

				//找到新的叶子节点的ebuffer_id
				ebuffer->size += 1;
				int target_bid = ebuffer->size;
				//cout<<"ebuffer size:"<<target_bid<<endl;
				// TODO ebuffer size
				
        		bool do_insert = false; // 确定执行更新还是插入操作
        		if (ebuffer->size >= EBUFFER_NUM_MAX_SIZE) { // 缓冲区满
            		target_bid = ebuffer->SelectVictim(); //选择要插入的bufferid

            		if(ebuffer->ebuf_des[target_bid].dirty){ //写入MBuffer
                		ebuffer->WriteBuffer(target_bid);
						ebuffer->node2buffer.erase(ebuffer->buffer2node[target_bid]);
                		ebuffer->mbuffer->UnfixPage(ebuffer->buffer2node[target_bid]);
            		...
            		target_bid = ebuffer->size;//要插入的bufferid
            		do_insert = true;
        		}

				//插入ebuffer中
				int node_id=_r->node_id;
				cout<<"新分裂出的叶子nodeid："<<node_id<<endl;
				memcpy(ebuffer->ebuf_pool[target_bid].field,_r,EBUFFER_SIZE);
				ebuffer->ebuf_des[target_bid].update(node_id, target_bid, 0, false);
        		ebuffer->SetDirty(target_bid);
				do_insert ? ebuffer->LRUInsert(target_bid) : ebuffer->LRUUpdate(target_bid);
            	//do_insert ? ebuffer->size += 1 : 0;
...
/*
				...
				//插入ebuffer中
				int node_id=_r->node_id;
				cout<<"新分裂出的内部nodeid："<<node_id<<endl;
				memcpy(ebuffer->ebuf_pool[target_bid].field,_r,EBUFFER_SIZE);
				ebuffer->ebuf_des[target_bid].update(node_id, target_bid, 0, false);
        		ebuffer->SetDirty(target_bid);
				do_insert ? ebuffer->LRUInsert(target_bid) : ebuffer->LRUUpdate(target_bid);
...
*/
			...
			//共同部分 更新父节点
			{
				...
				if (!p) {//产生新的父亲节点
					...
					cout<<"上溢码:"<<k<<endl;
					int i = 0; while (p->child[i] != v){cout<<i<<endl;++i;}
...
*/
					...
					//BAddTreeLeafNode<K, E>* te = (BAddTreeLeafNode<K, E>*) ebuffer->ebuf_pool[target_bpid].field;
					//cout<<"child nodeid:"<<te->child[1]->node_id<<endl;
					// TODO 更新地址转换map
					...
					cout<<"老父亲更新！"<<endl;		
					//cout<<"p child size:"<<p->child.size()<<endl;	
...
/*
					...
					int old_vid = v->node_id;
					int old_bvid = ebuffer->node2buffer[old_vid];
					cout<<"旧的叶子nodeid,bufferid: "<<old_vid<<","<<old_bvid<<endl;
					memcpy(ebuffer->ebuf_pool[old_bvid].field,v,EBUFFER_SIZE);
					//ebuffer->ebuf_des[target_bid].update(node_id, target_bid, 0, false);
					ebuffer->SetDirty(old_bvid);
					ebuffer->LRUUpdate(old_bvid);
