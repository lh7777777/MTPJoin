...
public:
    ...
    void WriteBuffer(int buffer_id,MB* mb) {
        ...
        ocall_writepage(buffer2page[buffer_id], mb->mbuf_pool[buffer_id].field);
    }
    // 从磁盘读一个page到mbuffer
...
public:
    ...
    int FixPage(int node_id, EBuf_pool &thebuffer,MB* mb) {
        ...
        if (page2buffer.find(page_id) != page2buffer.end()) { // 在缓冲区
            ...
            BAddTreeLeafNode<K, E>* node = (BAddTreeLeafNode<K, E>*) thebuffer.field;
			printf("nodeid: %d\n",node->node_id);
        ...
        if (page2buffer.find(page_id) != page2buffer.end()) { // 在缓冲区
            ...
            if (size >= MBUFFER_NUM_MAX_SIZE) { // 缓冲区满
                ...
                if (mb->mbuf_des[target_bid].dirty) { // dirty
                    printf("踢入disk\n");
                    ocall_writepage(buffer2page[target_bid], mb->mbuf_pool[target_bid].field);
    ...
    void UnfixPage(int node_id,MB* mb) {
        ...
        if (page2buffer.find(page_id) != page2buffer.end())
            mb->mbuf_des[page2buffer[page_id]].count -= 1;
    }
    // 获取命中率
    double GetHitRate() {
    ...
    void WriteDirtys(MB* mb) {
        ...
        for (int bid = 0; bid < size; bid++) {
            if (mb->mbuf_des[bid].dirty) {
                ocall_writepage(buffer2page[bid], mb->mbuf_pool[bid].field);
