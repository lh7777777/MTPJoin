...
namespace BAT {
	...
	public:
		...
		virtual E* find(K const &,void* mb);//查找内容
		virtual bool insert(E const&,void* mb);//插入
	...
	protected:
		...
		void solveOverFlow(BAddTreeLeafNode<K, E>*,MB* mb);//解决插入上溢现象 参数为节点
	...
	template<typename K, typename E>
	inline E* BAddTree<K, E>::find(K const &k,void* mb)
	{
			...
			if (ebuffer->node2buffer.find(node_id) != ebuffer->node2buffer.end()) { // 在缓冲区
				...
				if (ebuffer->size >= EBUFFER_NUM_MAX_SIZE) { // 缓冲区满
					...
					if (ebuffer->ebuf_des[target_bid].dirty == 1) { // dirty
						ebuffer->WriteBuffer(target_bid,mb);
				...
				ebuffer->ReadPage(node_id,tmp_buffer,mb);
		...
		if (v&&v->isLeaf()) {
			...
			if (ebuffer->node2buffer.find(node_id) != ebuffer->node2buffer.end()) { // 在缓冲区
				...
				// 读数据
				EBuf_pool tmp_buffer;
				ebuffer->ReadPage(node_id,tmp_buffer,(MB*)mb);
				ebuffer->ebuf_pool[target_bid].set_field(tmp_buffer);
	...
	template<typename K, typename E>
	inline bool BAddTree<K, E>::insert(E const &e,void* mb)
	{
			...
			v->e.insert(v->e.begin() + i, _e);//插入记录
				...
				if(size() == 1){//插入首个key
					...
					if (ebuffer->node2buffer.find(node_id) != ebuffer->node2buffer.end()) { // 在缓冲区
						...
						//node = (BAddTreeLeafNode<K, E>*) ebuffer->ebuf_pool[bid].field;
						//printf("找到的叶子nodeid:  %d\n",node->node_id);
					}
					else{
						// 不在缓冲区
						...
						if (ebuffer->size >= EBUFFER_NUM_MAX_SIZE) { // 缓冲区满
							...
							if (ebuffer->ebuf_des[target_bid].dirty == 1) { // dirty
								...
								ebuffer->WriteBuffer(target_bid,(MB*)mb);
								ebuffer->node2buffer.erase(ebuffer->buffer2node[target_bid]);
							}
						} else { // 缓冲区未满
							target_bid = ebuffer->size;
	...
	template<typename K, typename E>
	inline void BAddTree<K, E>::solveOverFlow(BAddTreeLeafNode<K, E>*v,MB* mb)
	{
				...
				if (!p) {//产生新的父亲节点
					...
					if (ebuffer->size >= EBUFFER_NUM_MAX_SIZE) { // 缓冲区满
						...
						if(ebuffer->ebuf_des[target_bpid].dirty){ //写入MBuffer
							...
							ebuffer->WriteBuffer(target_bpid,mb);
					...
					if (ebuffer->size >= EBUFFER_NUM_MAX_SIZE) { // 缓冲区满
						...
						if(ebuffer->ebuf_des[target_brid].dirty){ //写入MBuffer
							...
							ebuffer->WriteBuffer(target_brid,mb);
