...
public:
    ...
    void WriteBuffer(int buffer_id,void* mbpool) {
...
public:
    ...
    int FixPage(int node_id, EBuf_pool &thebuffer,MBuf_des* mbdes,void* mbpool) {
        MBuf_id mbuf_id;
    ...
    // 更新某个page
    void UpdatePage(int node_id, EBuf_pool &thebuffer,MBuf_des* mbdes,void* mbpool) {
    ...
    void WriteDirtys(MBuf_des* mbdes,void* mbpool) {
        ...
        for (int bid = 0; bid < size; bid++) {
            if (mbdes[bid].dirty) {
                ocall_writepage(buffer2page[bid], (char**)mbpool[bid]);
