...
namespace BAT {
	...
	public:
		...
		virtual E* find(K const &,MB* mb);//查找内容
		virtual bool insert(E const&,MB* mb);//插入
	...
	template<typename K, typename E>
	inline E* BAddTree<K, E>::find(K const &k,MB* mb)
	{
		...
		if (v&&v->isLeaf()) {
			...
			if (ebuffer->node2buffer.find(node_id) != ebuffer->node2buffer.end()) { // 在缓冲区
				...
				ebuffer->ReadPage(node_id,ebuffer->ebuf_pool[target_bid],mb);
	...
	template<typename K, typename E>
	inline bool BAddTree<K, E>::insert(E const &e,MB* mb)
	{
