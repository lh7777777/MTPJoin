#include "../Include/ds_mgr.h"

void DataStorageMgr::OpenFile(string filename, bool write) {
...
public:
    ...
    // 读取某个页
    int ReadPage(int page_id, MBuf_pool &buffer) {
        ...
        Seek(target_offset);
        FRead(buffer.field, 1, MBUFFER_SIZE, currFile);
        return 0;
    }
    // 更新某个页，页page_id一定存在，对于新加入的页，调用 [WriteNewPage]
    int WritePage(int page_id, MBuf_pool &buffer) {
        //LOG_DEBUG("DataStorageMgr.WritePage, page_id", page_id);
        int num_skip = page_id / PAGE_NUM_IN_CONTENT;
        SkipContent(num_skip);
        off_t target_offset = contents[page_id % PAGE_NUM_IN_CONTENT];
        Seek(target_offset);
        FWrite(buffer.field, 1, MBUFFER_SIZE, currFile);
        return 0;
    }
    // 写入一个新页
    int WriteNewPage(MBuf_pool &buffer) {
